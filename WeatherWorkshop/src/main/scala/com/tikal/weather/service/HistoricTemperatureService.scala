...
//      val allMonth : List[HistoricalDataMongo] = asScalaBuffer(x).toList
      val allMonth : List[HistoricalDataMongo] = findDataForStation(stationId, month, year) ;
...
//    "['1935', 34.0]" +
  ...
  def findDataForStation(stationId : String, month : Int, year: Int) : List[HistoricalDataMongo] = {
  ...
  def findDataForStation1(stationId : String, month : Int, year: Int) = {
      val x = dao.findByStationIdAndMonthAndYear(stationId, month, year)
      val y = if (stationId.equals("7150")) {
        dao.findByStationIdAndMonthAndYear("7151", month, year)
      } else if (stationId.equals("4640")) {
        dao.findByStationIdAndMonthAndYear("4642", month, year)        
      } else if (stationId.equals("5360")) {
        dao.findByStationIdAndMonthAndYear("5358", month, year)        
      } else new ArrayList()
      logger.info ( s"retrieved ${x.size()+y.size()} items for $month $year");
      val list = asScalaBuffer(y).toList ;
      val allMonth : List[HistoricalDataMongo] = asScalaBuffer(x).toList
      allMonth ++ list
  }
